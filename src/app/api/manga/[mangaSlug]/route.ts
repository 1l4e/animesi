...
export async function GET(request: Request,{params}:any) {
    try {
      ...
      // Select the elements that contain manga information using the provided selector
        ...
        const mangaInfo: any = {};
        const watch = obj.manga.detail.watchButton
        // Extract manga name from the provided selector
        mangaInfo.name = $(obj.manga.detail.name).text().trim();
        mangaInfo.author = $(obj.manga.detail.author).text().trim();
        mangaInfo.otherName = $(obj.manga.detail.otherName).text().trim();
        mangaInfo.description = $(obj.manga.detail.description).text().trim();

        

        mangaInfo.genres = []
        $(obj.manga.detail.genres).each((idd,genresItem)=> {
        ...
        mangaInfo.image = $(obj.manga.detail.image).attr('src');
        const type = obj.manga.detail.type;
        if (type){
        ...
        const chapterNameSelector = obj.manga.detail.chapter.chapterName
        const parent = obj.manga.detail.chapter.chapterUrl;
        const updatedTimeSelector=obj.manga.detail.chapter.updatedTime;
        mangaInfo.watch = $(watch).attr('href');
        if (mangaInfo.watch){
            const uuu = new URL(mangaInfo.watch);
            const path = uuu.pathname.replace('/xem','');
            mangaInfo.watch= path + `?type=chapter`
        }

        // Extract the list of latest chapters
        mangaInfo.chapters = [];
        $(obj.manga.detail.chapter.chapterList)
